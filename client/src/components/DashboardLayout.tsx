...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  return <div className="dashboard-shell"><aside className="dashboard-sidebar"><button className="dashboard-brand" onClick={() => setLocation("/")}><span className="brand-orb"><span /></span> Kuppi</button><div className="dashboard-nav"><button className={location === "/dashboard" ? "dashboard-nav-active" : ""} onClick={() => setLocation("/dashboard")}><LayoutDashboard size={17} /> Overview</button><button onClick={() => setLocation("/")}><BookOpen size={17} /> Discover resources</button>{student.role === "admin" && <button className={location === "/admin/moderation" ? "dashboard-nav-active" : ""} onClick={() => setLocation("/admin/moderation")}><ShieldCheck size={17} /> Moderation</button>}</div><div className="dashboard-user"><span>{student.fullName.split(" ").map((part) => part[0]).join("").slice(0, 2)}</span><div><strong>{student.fullName}</strong><small>@{student.username}</small></div></div><button className="dashboard-signout" onClick={() => logoutMutation.mutate()}><LogOut size={16} /> Sign out</button></aside><main className="dashboard-main">{children}</main></div>;
}
