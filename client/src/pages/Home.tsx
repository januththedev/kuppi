...
function TagChips({ tags, activeTag, onSelect }: { tags?: string[]; activeTag?: string | null; onSelect: (tag: string) => void }) {
  if (!tags?.length) return null;
  return <div className="tag-chip-row">{tags.map((tag) => <button key={tag} type="button" className={`tag-chip${activeTag === tag ? " tag-chip-active" : ""}`} onClick={(event) => { event.stopPropagation(); onSelect(tag); }}>#{tag}</button>)}</div>;
}
...
function ResourceCard({ resource, index, onOpen, onLike, onSave, activeTag, onSelectTag }: { resource: Resource; index: number; onOpen: () => void; onLike: () => void; onSave: () => void; activeTag?: string | null; onSelectTag: (tag: string) => void }) {
  ...
  return <motion.div initial={{ opacity: 0, y: 26 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-40px" }} transition={{ duration: 0.5, delay: Math.min(index * 0.06, 0.3), ease: "easeOut" }}>
    <article className="resource-card real-resource-card tilt-card" ref={tiltRef}>
      ...
      <div className="resource-card-body">
        ...
        <TagChips tags={resource.tags} activeTag={activeTag} onSelect={onSelectTag} />
...
export default function Home() {
  usePageTitle("Kuppi — Free Study Notes & Past Papers Shared by Sri Lankan Students");
  const [, setLocation] = useLocation();
  const heroTiltRef = useTilt<HTMLDivElement>(5);
  const utils = trpc.useUtils();
  const [query, setQuery] = useState("");
  // Deep link support: /?tag=thermal-physics pre-filters the feed to that
  // auto-generated hashtag (chips on cards and the permalink page link here).
  const [activeTag, setActiveTag] = useState<string | null>(() => new URLSearchParams(window.location.search).get("tag")?.replace(/^#/, "").toLowerCase() || null);
  ...
  const filters = useMemo(() => ({ query: query || undefined, subject, studyLevel, tags: activeTag ? [activeTag] : undefined }), [query, subject, studyLevel, activeTag]);
  ...
  function finishUpload(created?: unknown) {
    setUploadBusy(false);
    setUploadPercent(null);
    setShowUpload(false);
    setUpload({ title: "", description: "", subject: "Combined Maths", studyLevel: "A/L", stream: "", examRelevance: "", file: null });
    setHtmlPaste("");
    void utils.resource.list.invalidate();
    void utils.dashboard.mine.invalidate();
    toast.success("Your resource is now live in Kuppi.");
    // Publish flows hand back the stored resource — open it so students see
    // their material rendered the moment the upload lands.
    if (created && typeof (created as Resource).id === "number") {
      setSelectedResource(created as Resource);
    }
  }
  const storageModeQuery = trpc.storage.mode.useQuery();
  const [uploadBusy, setUploadBusy] = useState(false);
  const [uploadPercent, setUploadPercent] = useState<number | null>(null);
  const createMutation = trpc.resource.create.useMutation({ onSuccess: (created) => { setUploadBusy(false); setUploadPercent(null); finishUpload(created); }, onError: () => { setUploadBusy(false); setUploadPercent(null); toast.error("Kuppi could not publish that resource right now."); } });
  const uploadUrlMutation = trpc.resource.uploadUrl.useMutation({ onError: (error) => toast.error(error.message) });
  const createMetaMutation = trpc.resource.createMeta.useMutation({ onSuccess: (created) => finishUpload(created), onError: (error) => toast.error(error.message) });
  ...
  async function submitUpload(event: FormEvent) {
    event.preventDefault();
    if (uploadMode === "html") {
      const source = htmlPaste.trim();
      if (!source) return toast.error("Paste some HTML first, or switch back to attaching a file.");
      if (source.length > 15 * 1024 * 1024) return toast.error("Pasted HTML must stay under 15 MB.");
      setUploadBusy(true);
      let binary = "";
      new Uint8Array(new TextEncoder().encode(source)).forEach((byte) => { binary += String.fromCharCode(byte); });
      createMutation.mutate({
        title: upload.title, description: upload.description, subject: upload.subject, studyLevel: upload.studyLevel,
        stream: upload.stream || undefined, examRelevance: upload.examRelevance || undefined,
        originalFileName: `pasted-page-${Date.now()}.html`, mimeType: "text/html", dataBase64: btoa(binary),
      });
      return;
    }
    if (!upload.file) return toast.error("Attach the resource you want to share.");
    if (upload.file.size > 30 * 1024 * 1024) return toast.error("Files must be 30 MB or smaller.");
    ...
    reader.onload = () => {
      ...
      if (!dataBase64) { setUploadBusy(false); return toast.error("Kuppi could not read that file."); }
      createMutation.mutate({ ...meta, dataBase64 });
    };
    reader.onerror = () => { setUploadBusy(false); toast.error("Kuppi could not read that file."); };
