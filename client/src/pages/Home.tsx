...
export default function Home() {
  ...
  const createMutation = trpc.resource.create.useMutation({ onSuccess: () => { setUploadBusy(false); setUploadPercent(null); finishUpload(); }, onError: () => { setUploadBusy(false); setUploadPercent(null); toast.error("Kuppi could not publish that resource right now."); } });
  const uploadUrlMutation = trpc.resource.uploadUrl.useMutation({ onError: (error) => toast.error(error.message) });
  const createMetaMutation = trpc.resource.createMeta.useMutation({ onSuccess: finishUpload, onError: (error) => toast.error(error.message) });
